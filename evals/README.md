...
```bash
# 단일 Task 실행
python -m backend.evals run evals/tasks/workflow/my-task.yaml

# Suite 실행
python -m backend.evals run-suite evals/suites/regression/workflow-regression.yaml

# CI 게이트 체크
python -m backend.evals gate --suite workflow_regression
```
